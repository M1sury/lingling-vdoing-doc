...
首先我们是在引入了`Nacos-discovery`依赖之后，才拥有了自动注册的功能。而且作为`C/S`架构，不可能是`Server`端去主动连接`Client`端，因为`Client`端可太多了，没办法确定`IP`和端口。所以肯定是`Client`端去向`Server`端发起连接的请求。
...
从这里，我们再看`spring-cloud-starter-alibaba-nacos-discovery`依赖里面所有的类也大概不超过五十个，所以可以大概确定，我们上面所创建的第一个`Bean`对象就是`Nacos`服务注册的注册器。但是这个方法是从哪里调用？这里不太容易能看出来。再看看它的入参`Registration`，点进去是个接口，只有一个实现类`NacosRegistration`，但是它的属性和我们要解决的问题似乎搭不上边就先不看了。继续看第三个`Bean`
...
一看这个名字和自动注册相关，并且这个类里面也有`register`方法。这个`Bean`一定比较重要。从这个类里面看不出来可以去它的父类中找一下。它的父类有一个事件监听的方法`bind()`，监听的是`WebServerInitializedEvent`的事件，并且其中就会调用到`register`方法。

可以得知，自动注册的功能是利用`Spring`可以事件监听，在`Spring`容器启动时，发布事件，可以监听到，从而执行该注册方法
